...
/*   Updated: 2022/03/12 17:15:15 by rmoriya          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_read_line(char *save, int fd)
{
	...
	while (gnl_ft_strchr(save, '\n') == NULL)
	{
		...
		if (res == 0)
			break ;
		buff[res] = '\0';
		save = ft_free_pass(save, gnl_ft_strjoin(save, buff));
	}
	free(buff);
	return (save);
}
...
static bool	divide_line(char **save, char **line)
{
	...
	line_ptr = gnl_ft_strchr(*save, '\n');
	if (line_ptr != NULL)
		front_len = line_ptr - *save + 1;
	else
		front_len = gnl_ft_strlen(*save);
	*line = gnl_ft_substr(*save, 0, front_len);
	ptr_len = gnl_ft_strlen(line_ptr);
	*save = ft_free_pass(*save, gnl_ft_substr(*save, front_len, ptr_len));
